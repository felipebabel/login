...
function ValidateCodeEmailVerification() {
  ...
  const handleValidateCode = async () => {
    ...
    if (codeValue.length !== 6) {
      setError(t("validateCode.errorIncomplete"));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setError(t("validateCode.errorInvalid"));
        return;
      }
      ...
      setError(t("validateCode.errorServer"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="main-wrapper">
      ...
      <div className="form-container">
        ...
        <h2>{t("validateCode.title")}</h2>

        <p className="create-account-text">
          {flow === "reset-password"
            ? t("validateCode.enter6codesenttotemail")
            : t("validateCode.checkYourEmail")}
          <b> {maskEmail(email)}</b>
        </p>
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? t("validateCode.validating") : t("validateCode.verifyCode")}
          </button>
        </form>
      </div>
    </div>
